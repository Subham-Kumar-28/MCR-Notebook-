...
async function seedAdminCloud() {
    ...
    try {
        ...
        const needsRefresh = !existing.password_hash || !bcrypt.compareSync(adminPassword, existing.password_hash);
        if (needsRefresh || !existing.is_admin) {
            await cloud.updateUserPassword(existing.id, hash);
            await cloud.updateUserAdminStatus(existing.id, true);
            console.log('👑 Cloud admin password refreshed:', ADMIN_EMAIL);
        }
    } catch (e) {
