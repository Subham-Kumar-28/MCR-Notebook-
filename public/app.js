...
(function () {
    ...
    function showAuthMessage(elementId, msg, type) {
        var box = $(elementId);
        if (!box) return showToast(msg, type);
        box.textContent = msg;
    ...
    function applyTheme() {
        var theme = localStorage.getItem('notes_theme') || 'light';
    ...
    function setupEvents() {
        ...
        $('forgotPasswordBtn').addEventListener('click', function () {
            window.location.href = 'mailto:myselfsubhamkumar@gmail.com?subject=Password%20Reset%20Request%20for%20MCR%20Notebook';
        });
        ...
        $('loginBtn').addEventListener('click', function () {
            ...
            if (!email || !password) {
                showAuthMessage('authMessage', 'Please enter your email and password.', 'error');
            ...
            apiFetch(API.login, {
                ...
                var message = e.message || 'Invalid username or password.';
                if (message.indexOf('Invalid') !== -1 || message.indexOf('username') !== -1 || message.indexOf('password') !== -1) {
                    message = 'Invalid username or password. Please check your details or contact support';
                }
                showAuthMessage('authMessage', message, 'error');
        ...
        $('registerBtn').addEventListener('click', function () {
            ...
            if (!name) { showAuthMessage('registerMessage', 'Please enter your name.', 'error'); showToast('Please enter your name', 'error'); return; }
            if (!email) { showAuthMessage('registerMessage', 'Please enter your Gmail address.', 'error'); showToast('Please enter your Gmail', 'error'); return; }
            if (password.length < 6) { showAuthMessage('registerMessage', 'Password must be at least 6 characters long.', 'error'); showToast('Password must be at least 6 characters', 'error'); return; }
            clearAuthMessage('registerMessage');
            this.disabled = true;
            apiFetch(API.register, {
                ...
                showAuthMessage('registerMessage', e.message, 'error');
        ...
        $('menuSupport').addEventListener('click', function () {
            window.location.href = 'mailto:myselfsubhamkumar@gmail.com?subject=MCR%20Notebook%20Support%20Request';
        });
