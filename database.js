...
function seedAdmin() {
    ...
    const existing = queryOne('SELECT id, password_hash FROM users WHERE email = ?', [ADMIN_EMAIL]);

    if (existing) {
        // Keep the admin password synced with the current environment value.
        const shouldRefresh = !existing.password_hash || !bcrypt.compareSync(adminPassword, existing.password_hash);
        if (shouldRefresh) {
            db.run('UPDATE users SET password_hash = ?, is_admin = 1 WHERE email = ?', [hash, ADMIN_EMAIL]);
            persist();
            console.log('👑 Admin password refreshed for:', ADMIN_EMAIL);
        } else {
